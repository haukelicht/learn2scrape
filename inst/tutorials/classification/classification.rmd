...
We have discussed that we need to split the sample into a training and a test set. Here, we will just use `quanteda`'s in-built sampling function. However, we will discuss how to sample in other packages (and how to sample non-text data) at the end of this tutorial.

### Sampling in quanteda

quanteda comes with a `dfm_sample()` function. With this, you can easily create a random training sample - you just need to decide on the size. In our case, I suggest to put most data into the training sample as we anyway do not have so much information to learn from. 
...
Then, you can also restrict the features of the test set to those features mentioned in the training set - otherwise, your classifier will not work. For this, you can use `dfm_keep()` (or `dfm_select()` with specifying that selection="keep" - the first variant is just a short-hand). Basically you just need to remember the difference between `dfm_trim()` which cuts features based on their frequency and `dfm_keep` which changes features based on their presence in a character vector or another dfm.
...
However, be a bit cautious with the interpretation of your results: ultimately, every run is a bit random and accuracy will vary depending on the sample - so look at the confidence interval. Also, this might be very different for other data sets and classifiers so take your time to practice with other data as well.

*If you run the classifiers a few times, you will see that the results vary a lot. This is because in the end, we have a quite small training and test set!*
...
While many machine learning algorithms are a black box, it is relatively easy to see which features are most important in the determination of each class. You can use `coef()` to see the coefficients of each feature.
...
Have a look at the 20 features which are most associated with the conflict section (CF)! *If you do not want to copy the model code again, I have pre-estimated a classifier called* `nb_model`. Are the features similar to what you thought of for the dictionary?
...
If you think you would like to try more classifiers, `caret` is your friend. Check the [list of models by tag](https://topepo.github.io/caret/train-models-by-tag.html) to look through the models caret has implemented. Just be aware that many of them take quite long since they are not optimized for text classification. Also, some have specific restrictions (e.g. they can only distinguish between two classes).

Some people use other packages like `RTextTools`, however, I do not recommend them since they are not maintained anymore and there is no support in case of problems. If you have previously used `tidytextt()`, I am also a fan of [this tutorial](https://juliasilge.com/blog/tidy-text-classification/).

Below, we just discuss the most important differences if you want to try your luck with caret.

### Sampling in caret

Sampling in `caret` is a bit more tedious - we will not go through it in detail, there are just two details I would like to point out: First, you have to convert the dfm into a data frame to do that. You can do that using `convert(dfm-name,"data.frame")`, so that is easy enough. Second, `caret` has a lot more options for sampling. So if you want to stratify your sample by groups (e.g. to over-sample an under-represented class to make sure your classifier learns it), it is worth getting acquainted with the function a bit more. If you are interested, just check the documentation below. Otherwise, it can be simpler to still do the sampling in quanteda so you won't have to deal with empty features later on.
...
The beauty of caret is that it unifies the syntax of lots of classification packages within R. The central function is `train()` and you can use it for different classifiers by just specifying a different method.


```{r caret-classify,exercise=TRUE}

```
```{r caret-classify-solution,eval=F}
