...
- You can test your code by clicking on *Run Code*. Since there is no environment, you can also add the names of objects to inspect them or use `ls()` to see what is in your environment when you execute the chunk
...
- unlike in a normal markdown document, all chunks are independent. That means: If you create something in one chunk, it won't be available in the other chunks
    - that means, you will sometimes have to copy-paste code from previous exercises but I have tried to make things as easy as possible
...
But don't worry, we will go step by step. Let's start with something simple. Imagine, we want to scrape this simple webpage full of quotes. Its address is [http://quotes.toscrape.com/](http://quotes.toscrape.com/){target="_blank"} Just have a look at the webpage.
...
The output does not look like you expected? You would be surprised to see that the [original webpage](http://quotes.toscrape.com/){target="_blank"} looks quite similar. Have a look at the webpage source code. Depending on your browser, you can probably select 'view source' after a right mouse click. If you have trouble, [google has an up-to-date explanation](https://www.google.com/search?hl=&site=&q=how+to+view+webpage+source+code+in+browser){target="_blank"}. 
...
Basically, HTML Tags form the structure of a page and CSS selectors are a type of *grammar* or *pattern-description* that helps us select specific parts of the page structure. I am working on a tutorial specific to CSS selectors (that you will be able to access by running `run_tutorial("CSS",package="learn2scrape")` in the future), but for now, we will just use a tool that helps us determine the correct selectors. In fact, most people use these tools for scraping and only learn the basics of CSS selectors.

Just for reference: For a list of CSS Selectors, check out [this collection](https://www.w3schools.com/cssref/css_selectors.asp){target="_blank"}. If you want to practice CSS Selectors in a fun way, I recommend playing with the [CSS Diner](https://flukeout.github.io/){target="_blank"} where you can learn about different selector structures.
