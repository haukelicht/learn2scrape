...
We will use the following R packaages:
...
- you cn check the HTTP status code of your request
...
Sessions an be created in `rvest` with the `session()` function (`html_session()` before version 2.0.0).
The only required parameter is `url`, the address of the webpage to request.

**Try it yourself!**
Create a session using https://scrapethissite.com/pages/simple/ as URL.
...
### Parsing HTML code from a ssession object
