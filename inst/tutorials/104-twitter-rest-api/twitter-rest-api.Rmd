...
Before getting started, we need to authenticate:

```{r authenticate-invisible, exercise = FALSE, include = FALSE}
...
**Try it yourself!**: Copy-past your API token information.

*Hint*: If you haven't yet obtained your API credentials, call `vignette("auth", package = "rtweet")` and follow the instructions.

```{r authenticate}
# create your rtweet API 'token' here
token <- create_token(
  # ToDo: add App name and secrets
  app = ...,
  consumer_key = ...,
  consumer_secret = ...,
  access_token = ...,
...
Now, we can again search for tweets with a geocode. Hopefully, when also including past tweets, we will receive more results, so we will try to do this with Tweets 50km around *Central European University* (CEU).
...
# search for tweets created (by users) near CEU
ceu <- search_tweets(geocode = "47.500586,19.049482,50km", n = 100)

# convert to latitude and longitude
ceu <- rtweet::lat_lng(ceu)

# plot
maps::map("world", regions = "Hungary")
with(ceu, points(lng, lat, col = rgb(0, .3, .7, .75)))
```
...
The opposite approach is to search for lists that contain a certain user with `lists_memberships()`. E.g., you could look for all lists that contain Donald Trump.

```{r lists-memberships, exercise.lines = 1}
trump_lists <- lists_memberships(user = "realDonaldTrump")
```
