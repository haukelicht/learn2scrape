...
setNames(c(3, 2, 1.5), c("apple", "banana", "lemon"))
...
```{r creating-decreasing-sequences, exercise.lines = 2}
seq(-10, 10, by = -5L) # doesn't work
seq(10, -10, by = -5L) # works!
```
...
- `for` tells R that you want repeat some code ("iterate")
- you write the code you want to repeat *inside* the curly braces (i.e., between `{` and `}`)
- how often R evaluates this code is determined by the expression `(i in VECTOR)`
...
Look at the following example and note that this code runs smoothly, too.
This shows that instead of  `i`, you can use a different word --- the word is is just a placeholder!
...
1. you create an *empty* vector/list (e.g. using `integer()`, or `list()`)
2. whenever you generate a valid value in an iteration, you "append" it by assigning it the the end of the vector/list.

Appending to the last position can be done by assigning a value to the position of the vector/list that comes after the actual end of this vector, that is, at position `length(vector)+1`:
...
Two important features of `for`-loops are that, if desired, you can *skip* iterations or even *stop* the loop from continuing to iterate.
...
You can **interrupt/stop** a running `for`-loop with the `break` keyword:
...
```{r function-no-arg, exercise.eval = TRUE}
hello <- function() {
  print("Hello")
}
```
...
```{r  function-no-arg-call, exercise.setup = "function-no-arg", exercise.lines = 2}
hello <- function() { print("Hello") }
hello()
```

Note that this function works without us passing any inputs.
This is because our `hello()` function has no arguments (i.e., parameters based on which it adapts its behavior)
...
```{r function-one-arg-setup, exercise = FALSE, echo = FALSE}
hello <- function(name) {
  print(paste("Hello", name))
}
```
...
```{r function-one-arg-fail, exercise.setup = "function-one-arg-setup", exercise.lines = 1}
hello()
```

So we have to pass a value to the `name` argument.
We do this inside the parentheses behind the function name: 

```{r function-one-arg-success, exercise.setup = "function-one-arg-setup", exercise.lines = 1}
hello("Hauke")
```
...
*Note:* You could actually omit the return in the above function and instead write because whatever is evaluated last in a function is returned.
...
divide <- function(numerator, denominator) {
  if (denominator == 0) {
    print("exiting early because denominator is 0")
    return(NaN)
  }
    
  if (numerator == 0) {
    print("exiting early because numerator is 0")
    return(Inf)
  }
